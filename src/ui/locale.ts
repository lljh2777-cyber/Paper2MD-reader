...
const ENGLISH = {
  ...
  noOutlineHeadings: "No headings in this document",
  ...
  statusMineruVerified: "Verified MinerU package",
  statusMineruVerifiedPartial: "Verified package · some captions incomplete",
  statusMineruUnverified: "Unverified MinerU folder",
  ...
  webEmptyCopy: "Open a Paper2MD or MinerU result folder, a standalone Markdown document, or a saved web clipping. Text and visuals are prepared locally in this browser.",
  webEmptyNote: "Read-only · no upload · generated Figure labels exist only in the reading view",
  webProcessingNote: "Local folders stay in this browser · without a configured service, PDF text projection also runs locally · generated Figure labels are display-only",
  ...
  openWebClipping: "Import paper package / ZIP",
  selectedWebClippingOpenFailed: "The local paper package could not be opened. Select one Paper2MD ZIP, Markdown, or HTML document with its local image files.",
  ...
  specifyCorrectCaptionLink: "Specify the correct cross-page caption link",
  captionCorrectionHelp: "Select one existing visual block on the source page and one or two existing text blocks on the immediately following page. Page order, geometry, Figure key, and exact Markdown ranges are revalidated before display.",
  captionSourceVisual: "Source visual",
  captionTargetBlocks: "Caption text on page {page}",
  textBlockLabel: "Text block {order}",
  submitCorrectCaptionLink: "Validate and apply relationship",
...
const CHINESE: Record<ReaderMessageKey, string> = {
  ...
  statusMineruVerified: "MinerU 内容包已验证",
  statusMineruVerifiedPartial: "内容包已验证 · 部分图注不完整",
  statusMineruUnverified: "MinerU 目录未验证",
  ...
  webEmptyCopy: "可打开 Paper2MD / MinerU 结果文件夹、Markdown 文档或已保存的网页剪藏；正文与图表均在当前浏览器本地整理。",
  webEmptyNote: "只读 · 不上传 · 自动补充的 Figure 编号仅存在于阅读显示层",
  webProcessingNote: "本地目录只在浏览器内读取 · 未配置处理服务时，PDF 文本投影也在本机完成 · Figure 编号仅存在于显示层",
  webNoFolder: "尚未选择文件夹",
  ...
  openWebClipping: "导入论文包 / ZIP",
  selectedWebClippingOpenFailed: "无法打开网页剪藏。请选择一个 Markdown 或 HTML 文档，并一并选择其本地图片文件。",
  ...
  specifyCorrectCaptionLink: "重新指定跨页图注关系",
  captionCorrectionHelp: "从来源页选择一个已有视觉块，并从紧邻下一页选择一至两个已有文本块。显示前会重新检测页序、坐标、Figure 编号和 Markdown 精确区间。",
  captionSourceVisual: "来源视觉块",
  captionTargetBlocks: "第 {page} 页的图注文本块",
  textBlockLabel: "文本块 {order}",
  submitCorrectCaptionLink: "检测并应用关系",
