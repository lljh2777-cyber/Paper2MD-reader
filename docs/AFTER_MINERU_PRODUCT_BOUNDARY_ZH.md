...
- `sidecars/`：Reader projection、viewer index、visual repair、visual candidates、provenance、validation、可选的 `repair-report.json`、`ATTRIBUTION.md` 与 v0.1.3 兼容记录。署名存在时必须同时提供哈希绑定的 `_source/ATTRIBUTION.md` 兼容别名；它是独立元数据，不属于或改写 MinerU source tree。
...
## `after-mineru-portable-markdown-v1` / `v2`

通用 Markdown ZIP 与可验证论文包是两个独立输出，portable 契约不改变 `after-mineru-package-v1` 或桌面版 v0.1.3 ABI。无署名元数据时继续输出严格、字节兼容的 `after-mineru-portable-markdown-v1`；经过 formal 包验证的署名存在时输出 `after-mineru-portable-markdown-v2`，并增加 manifest 绑定的根目录 `ATTRIBUTION.md`。除该可选署名外，portable 只包含根目录的 `article.after-mineru.md`、正文实际引用的本地图片，以及 `after-mineru-portable.json`；不包含源 ZIP、PDF、MinerU JSON、Reader sidecar 或交互样式，因此不建立 Reader 信任语义。v1 不接受 attribution 字段，v2 必须携带 attribution 字段和文件，两个版本均使用精确字段与文件库存验证。
...
站点通用 Reader、首页嵌入 Reader、内存预览与 DebyeCalculator 真实示例均使用 `strict-readonly` 能力档位：旧版 v0.1.3 MinerU 包只显示原始结构化内容，不在运行时应用 legacy sidecar、用户 review 或 PDF 文字恢复；正式 `after-mineru-package-v1` 在完整验证后显示物化派生正文。DebyeCalculator 示例现由内容寻址的 formal v1 包驱动，每次原始预览和派生 Reader 都获得独立的临时文件系统；原始视图从 formal 包内字节不变的 MinerU ZIP 重新导入，派生视图只消费 `verified-derived` 投影。原 v0.1.3 包继续作为单独下载的兼容资产保留，不再充当示例 Reader 的运行时信任根。桌面 v0.1.3 暂保留显式兼容档位及现有 sidecar IPC/文件契约。
