...
尚待继续迁移的是连续 PDF 页阅读、更复杂的跨栏/整段 PDF 文本层补全，以及人工/可选 AI 审核界面。这里所说的参考实现是独立的 Research Agent Reader Obsidian 插件及其配套转换规则；本仓库早期的 Paper2MD Reader 旧代码只是待替换实现，二者不是同一个项目。
