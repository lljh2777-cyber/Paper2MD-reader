...
| 正文页→PDF 页跟随及交互暂停 | 待迁移 | 下一阶段建立单一 reducer/state authority |
| PDF 版面框与点击定位 Figure | 待迁移 | 需要向共享模型暴露已验证 ViewerIndex 页面块 |
| PDF.js 空白图像兼容补绘 | 待迁移 | 仅使用已校验 MinerU 大图资产覆盖对应 bbox |
...
3. Markdown/PDF/Visual 单一权威同步状态机。
4. 版面框、PDF 图像兼容补绘及剩余确定性显示修复。
