...
| 人工候选裁决与结构化纠错 UI | 已迁移（碎图首版） | 可接受/拒绝/暂缓既有候选；拒绝后可从同页现有块重新指定组合，坐标与路径由验证包推导并再次检查，只写浏览器 sidecar |
| 可选 AI 候选裁决 | 待迁移 | AI 只能裁决已有 candidate_id；不得提交新坐标、路径、正文或未经检测的组合 |
...
6. 阅读状态持久化（已完成）。
7. 人工候选审阅与碎图结构化纠错（已完成首版）；继续跨页图注纠错、可选 AI 裁决与最终安全审计。
