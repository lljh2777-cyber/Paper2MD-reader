...
function inputHash(payload: UnknownRecord, name: string): string {
  ...
  return typeof value === "string" && /^[0-9a-f]{64}$/i.test(value) ? value.toLocaleLowerCase() : "";
}
...
function requiredInputHash(payload: UnknownRecord, name: string): { hash: string; error: "missing" | "invalid" | null } {
  ...
  return { hash: hashRecord.sha256.toLocaleLowerCase(), error: null };
}
...
function safeCodes(value: unknown): string[] {
  ...
  return [...new Set(value.map((item) => String(item ?? "").trim().toLocaleLowerCase()).filter((item) => item && CANDIDATE_SAFE_CODE.test(item)))].sort();
}
...
function fragmentCandidate(group: UnknownRecord, blocks: Map<string, CandidateBlockLocation>): UnknownRecord | null {
  ...
  return {
    ...
    evidence: {
      member_geometry: (geometry.filter(Boolean) as UnknownRecord[]).sort((left, right) => numberValue(left.page_order) - numberValue(right.page_order) || String(left.block_id).localeCompare(String(right.block_id))),
...
function captionCandidate(input: {
  ...
  return {
    ...
    evidence: {
      ...
      caption_geometry: (captionGeometry.filter(Boolean) as UnknownRecord[]).sort((left, right) => numberValue(left.page_order) - numberValue(right.page_order) || String(left.block_id).localeCompare(String(right.block_id))),
...
function ambiguousCaptionCandidates(
  ...
  for (const issueValue of visualRepair.issues) {
    ...
    if (!issue) continue;
    const issueCode = String(issue.code ?? "").trim().toLocaleLowerCase();
...
export function buildMineruVisualCandidates(viewerIndexValue: unknown, visualRepairValue: unknown, minimumAcceptConfidence = 0.85): UnknownRecord {
  ...
  const candidates = collector.values().sort((left, right) => String(left.kind).localeCompare(String(right.kind)) || String(left.candidate_id).localeCompare(String(right.candidate_id)));
